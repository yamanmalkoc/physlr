...
class Physlr:
    ...
    def determine_molecules(g, u):
        ...
        # Strategies summary:
        #    - strategy 1: outdated previous physlr method
        #    - strategy 2: physlr current
        #    - strategy 3:
        #       - strategy2 1:      calculate sqCos but report strategy 2 (cuurent physlr)
        #       - strategy2 1.5:    calculate sqCos (with blas.sgemm) but report strategy 2
        #       - strategy2 2:      sqCos with: np.dot / dict /
        #       - strategy2 2.5:    sqCos with: blas.sgemm / panda /
        ...
        if strategy == 3:
            ...
            if nodes_count == 0:  # or edges_count == 0:
                ...
                components = list(nx.connected_components(g.subgraph(set(g.neighbors(u)))))
                return u, {v: i for i, vs in enumerate(components) if len(vs) > 1 for v in vs}
            if strategy2 == 1:  # Just run sq and Cos but return current (time profiling)
                adj = nx.adjacency_matrix(sub_graph)
                ...
                if edges_to_keep.__len__() == -1:
                    print(",\n")
                cut_vertices = set(nx.articulation_points(g.subgraph(g.neighbors(u))))
                components = list(nx.connected_components(g.subgraph(set(g.neighbors(u)) - cut_vertices)))
                components.sort(key=len, reverse=True)
                return u, {v: i for i, vs in enumerate(components) if len(vs) > 1 for v in vs}
            if strategy2 == 1.5:  # Again like 1 but this time with sq with blas.sgemm
                adj_array = nx.adjacency_matrix(sub_graph).toarray()
                cos_args = np.argwhere(cosine_similarity(sp.linalg.blas.sgemm(1.0, adj_array, adj_array)) >= threshold)
                if cos_args.__len__() == -1:
                    print(",\n")
                cut_vertices = set(nx.articulation_points(g.subgraph(g.neighbors(u))))
                components = list(nx.connected_components(g.subgraph(set(g.neighbors(u)) - cut_vertices)))
                components.sort(key=len, reverse=True)
            ...
            if strategy2 == 3:
                ...
                edges_to_remove_barcode = [(barcode_dict[i], barcode_dict[j]) for i, j in edges_to_remove]
                subcomp2 = nx.Graph(sub_graph)
                subcomp2.remove_edges_from(edges_to_remove_barcode )
                # subcomp2 = nx.freeze(subcomp2)
                cos_components = list(nx.connected_components(subcomp2))
                cos_components.sort(key=len, reverse=True)
                return u, {v: i for i, vs in enumerate(cos_components) if len(vs) > 1 for v in vs}
    ...
    def parse_arguments():
        ...
        argparser.add_argument(
            "--bst", action="store", dest="bst", type=float, default=75,
        ...
        argparser.add_argument(
            ...
            help="exclude the barcode itself from the subgraph (0 or 1) [0]")
        argparser.add_argument(
